...
import { Heart, Maximize2, X, Terminal, Star, Download } from "lucide-react";
...
function PixelImage({ src, alt, className, fill, width, height, ...props }: PixelImageProps) {
...
function GalleryContent() {
  ...
  const getCount = useCallback((name: string) => wallpapers.filter(w => w.character === name).length, [wallpapers]);
  ...
  const renderCharacterName = (charName: string) => {
    ...
    return (
      <span className="inline-flex items-center gap-2">
        {formattedName}
        <img src={`/stickers/${stickerSlug}.png`} alt="" className="w-6 h-6 object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
      </span>
    );
  };
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                  {specialCollections.map((char) => (
                    <button 
                      ...
                      className={`px-6 py-3 rounded-xl transition-all border backdrop-blur-md ${filter === char ? "bg-neon-cyan/20 border-neon-cyan text-white scale-105" : "bg-white/5 border-white/10 text-gray-300 hover:border-neon-cyan/50 hover:bg-white/10"}`}
                    >
          ...
          <div className="w-full max-w-6xl">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <button 
                ...
                className={`px-6 py-2 rounded-full font-pixel text-sm transition-all border ${filter === "All" ? "bg-neon-pink border-neon-pink text-white" : "bg-dark-bg/50 border-gray-700 text-gray-400"}`}
              >
                ALL <span className="ml-2 opacity-50">{everything.length}</span>
              </button>
              {favorites.length > 0 && (
                <button 
                  onMouseEnter={() => playSound('hover')} 
                  ...
                  className={`px-6 py-2 rounded-full font-pixel text-sm transition-all border flex items-center gap-2 ${filter === "Favorites" ? "bg-white text-dark-bg" : "bg-dark-bg/50 border-gray-700 text-neon-pink"}`}
                >
                  <Star size={16} fill={filter === "Favorites" ? "black" : "none"} /> FAVS ({favorites.length})
                </button>
              )}
              {characters.map(char => (
                <button 
                  ...
                  onClick={() => handleFilterChange(char)} 
                  aria-pressed={filter === char}
                  className={`px-5 py-2 rounded-full transition-all border text-sm flex items-center gap-2 ${filter === char ? "bg-neon-purple/20 border-neon-purple text-white shadow-[0_0_15px_rgba(211,0,197,0.4)] scale-105" : "bg-dark-bg/30 border-gray-800 text-gray-400 hover:border-neon-cyan/50"}`}
                >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-4 flex justify-between items-center bg-dark-bg/60 backdrop-blur-md rounded-b-lg border-t border-white/5">
                      ...
                      <button 
                        onClick={(e) => { e.stopPropagation(); toggleFavorite(wp); playSound('click'); }} 
                        className="p-2 hover:scale-125 transition-transform"
                        aria-label={isFavorite(wp.id) ? `Remove ${wp.title} from favorites` : `Add ${wp.title} to favorites`}
                      >
          ...
          {!loading && displayCount < filteredItems.length && (
            <div className="flex justify-center mt-12">
              <button onMouseEnter={() => playSound('hover')} onClick={handleLoadMore} className="font-pixel px-12 py-4 border-2 border-neon-cyan text-neon-cyan hover:bg-neon-cyan hover:text-black transition-all rounded-lg shadow-[0_0_20px_rgba(5,217,232,0.3)]">SHOW MORE WALLPAPERS</button>
            </div>
          )}
        </div>
      </section>
      ...
      {selectedImageIndex !== null && (
        <Lightbox 
          images={filteredItems.map(w => w.url)} 
          titles={filteredItems.map(w => w.title)} 
