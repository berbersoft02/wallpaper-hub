...
async function getSubFolders(root) {
  try {
    ...
    console.error(`Error getting subfolders for ${root}:`, e.message);
    return [];
  }
}
...
async function start() {
  const folders = await getSubFolders('wallpapers');
  console.log(`🚀 Starting standardization for ${folders.length} folders.`);

  for (const folder of folders) {
    await renameFilesInFolder(folder);
  }

  console.log('\n✅ All folders processed. RUNNING SYNC...');
}
